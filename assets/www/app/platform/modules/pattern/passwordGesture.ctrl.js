...
(function () {
    ...
    PatternPassWordGestureCtrl.$inject = ['$stateParams', '$rootScope', '$sessionStorage', '$localStorage', '$scope', '$state', '$ionicPopover','$ionicNativeTransitions', '$ionicLoading','UserService','$cordovaScanning', '$translate'];
    function PatternPassWordGestureCtrl($stateParams, $rootScope, $sessionStorage, $localStorage, $scope, $state, $ionicPopover,$ionicNativeTransitions, $ionicLoading, UserService, $cordovaScanning, $translate) {
        var vm = this;
        ...
        function checkPatternNoLogin(loginPattern) {
            $cordovaScanning.commont("devices", "").then(function (data) {
                ...
                //vm.uuid = '111111';
                $ionicLoading.show();
                    UserService.checkPatternNoLogin(vm.uuid,loginPattern).success(function (data) {
                        if(data.checkFlag == undefined){
                            ...
                            vm.numberState++;
                            $scope.$emit('alertWarningCallBack', $translate.instant('pattern.wrong') ,function(){
                                lock.reset();
                            });
                            if (vm.numberState >= 5){
                                vm.numberState = 0;
...
//                        lock.reset();
                    ...
                    $scope.$emit('alertWarningCallBack', $translate.instant('pattern.request'), function(){
                        lock.reset();
                    });
                }
            }
